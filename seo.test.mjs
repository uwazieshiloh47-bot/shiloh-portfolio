...
/*
  Each page carries its own Open Graph card now, so sharing a link to the
  projects page unfurls as Projects rather than as the generic homepage card.
  They are generated by scripts/social-cards.mjs; `card` here is the file that
  script writes for the page.
*/
const publicPages = [
  { file: "index.html", canonical: `${siteUrl}/`, card: "social-preview.png" },
  { file: "about.html", canonical: `${siteUrl}/about.html`, card: "social-about.png" },
  { file: "work.html", canonical: `${siteUrl}/work.html`, card: "social-work.png" },
  { file: "skills.html", canonical: `${siteUrl}/skills.html`, card: "social-skills.png" },
  { file: "resume.html", canonical: `${siteUrl}/resume.html`, card: "social-resume.png" },
  { file: "contact.html", canonical: `${siteUrl}/contact.html`, card: "social-contact.png" },
];
...
/*
  2400x1260 rather than 1200x630: the cards keep the documented Open Graph
  aspect ratio but are rendered at twice the density so they stay sharp on
  high-DPI screens. The meta tags report these real pixel dimensions.
*/
test("every social card is a 2400 by 1260 PNG", async () => {
  const cards = [...new Set(publicPages.map((page) => page.card))];
  assert.equal(cards.length, 6, "expected one card per public page");

  for (const card of cards) {
    const image = await readFile(card);
    ...
    assert.equal(image.readUInt32BE(16), 2400, `${card}: width`);
    assert.equal(image.readUInt32BE(20), 1260, `${card}: height`);
  }
});
